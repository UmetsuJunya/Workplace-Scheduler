...
- **勤務地背景色設定**: 勤務地ごとに背景色を設定（6つのプリセット色 + カスタムカラー）
...
- **認証機能**: メールアドレスベースのユーザー登録・ログイン（オプション）
...
**認証機能の動作**:
- **有効時 (`true`)**: アプリケーション起動時にログイン画面が表示され、メールアドレスとパスワードによる認証が必要です。ユーザー登録画面からアカウント作成できます。
- **無効時 (`false`)**: 認証なしで直接スケジュール画面にアクセスできます（デフォルト設定）。
...
3. **背景色の設定**: 勤務地管理画面で各勤務地の背景色を設定（6つのプリセット色またはカスタムカラー）
4. **スケジュール入力**: カレンダーのセルをクリックして勤務地を選択（設定した背景色で表示されます）
5. **絞り込み**: ユーザーやプロジェクトで表示をフィルタリング
6. **一括編集**: 「一括編集」ボタンで複数日をまとめて編集
